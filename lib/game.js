...
export function todayTotals(entries) {
  ...
  return {
    ...
    carbs: sum("carbs_g"),
    fat: sum("fat_g"),
    addedSugar: sum("added_sugar_g"),
    caffeine: sum("caffeine_mg"),
    produce: sum("fruit_veg"),
    nutritionConfidence: todays.map(entry => entry.data?.nutrition_confidence).filter(Boolean),
    calorieStatus: todays.map(entry => entry.data?.calorie_status).find(Boolean) || null,
    miles: sum("miles"),
    squats: sum("squats"),
    water: sum("water_oz"),
